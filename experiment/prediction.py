...
#Learn the BN parameters with libpgm
def features2array(Fdata):      
    """            
    This function converts a feature dataframe created with features.feature_extraction.genF() into an array.
    The array can be used in learnBN to learn the parameters of the Bayesian Network.
    """
    for c in Fdata.columns:
        Fdata[c].replace(np.nan, '', regex=True, inplace=True) #drop nan as this set obersatvions cannot be used to learn BN
    Fdata.drop(columns=['ProfileID','elec_bin'], inplace=True)
    featuredict = Fdata.to_dict('index') 
    e = []
